...
function onToggleKeyChange(pref) {
  workers.forEach(function(worker){
    worker.port.emit("update_prefs", {"key": sprefs.prefs.toggle_key});
  });
}
sprefs.on("toggle_key", onToggleKeyChange);
...
var selector = pageMod.PageMod({
  ...
  contentScriptOptions: {
      "key": sprefs.prefs.toggle_key,
      "status": sprefs.prefs.default_status
  },
  contentScriptFile: data.url('content.js'),
  onAttach: function(worker) {
    workers.push(worker);
    worker.port.on("ready", function() {
      var site_settings = manager.getSiteSettings(utils.grepUrl(worker.tab.url));
      var data = {
...
popup.port.on("get_hash", function(key) {
  var site_profile = manager.getSiteSettings(utils.grepUrl(currentUrl));
  var profile = manager.getProfile(site_profile.profile_index);
  var hash = passwordHasher.hashPassword(
    site_profile.tag,
    key,
