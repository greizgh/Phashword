...
function hasChange(orig_profile, new_profile) {
    var keys = ['profile_index', 'tag', 'status', 'password_type', 'password_length'];
    for (let key of keys) {
        if (orig_profile[key] != new_profile[key]) {
            return true;
        }
    }
    return false;
...
ProfileManager.prototype.getNewProfile = function() {
    return createDefaultProfile(_("new_profile"));
};
 ...
 * Set current profile
 */
ProfileManager.prototype.setCurrentProfile = function(profile) {
 ...
 * Save site profile
 */
ProfileManager.prototype.setSiteSettings = function(site, profile, force) {
    // Object value comparison
    if (hasChange(this.getSiteSettings(site), profile) || force) {
...
ProfileManager.prototype.getSiteSettings = function(site) {
    var site_settings = {
        ...
        "password_length": this.currentProfile.password_length,
        "password_type": this.currentProfile.password_type,
    };
